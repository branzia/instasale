...
(onboarding)/instagram-required — signed in, but !isInstagramConnected
  ...
  ↓
(tabs) — Home (+ Catalog and Settings, one tap away via header icons) / Posts & Reels / Automations / Leads (+Orders segment)
```
...
What actually shipped: **Home became a small stack** (`app/(tabs)/home/`, replacing the old flat `app/(tabs)/index.tsx` — hence `RootLayoutNav` now redirects to `/(tabs)/home` explicitly, since `/(tabs)` alone no longer resolves to anything) with two small icon buttons in its header — a basket icon and a gear icon — that push to `home/catalog.tsx` and `home/account-settings.tsx` respectively. Both screens' content is otherwise unchanged from the old `catalog/` and `settings/` tabs; `product-form.tsx`/`attribute-form.tsx` moved from `catalog/` to `home/` alongside `catalog.tsx`, registered as extra `Stack.Screen`s in `home/_layout.tsx`. `app/(tabs)/catalog/` and `app/(tabs)/settings/` are both gone.
